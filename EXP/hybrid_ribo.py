...
class LitHybrid(pl.LightningModule):
    ...
    def __init__(self, batch_size=32, readout='flattening'):
        ...
        self.lit_layers = gcnSeq('x, x_prime, edge_index, batch', [
            (CNN(in_ch=16, hidden_ch=(32, 64, 128)), 'x_prime -> x_cnn'),
            (GCN(in_channels=4, readout=readout), 'x, edge_index, batch -> x_gcn'),
            (CatLayer(), 'x_cnn, x_gcn -> x_cat'),
            (MLP(filters=[42240, 128, 64, 1], dropout=0.3), 'x_cat -> x_out')
        ])
        # 34048
        self.best_r2 = -np.inf

    def forward(self, data):
        x, edge_index, x_prime, batch = data.x, data.edge_index, data.mat, data.batch
        x_prime = x_prime.view(-1, 16, 148, 148)

        return self.lit_layers(x, x_prime, edge_index, batch)
...
def train_main_hybrid(params):
    ...
    ribonet = LitHybrid(batch_size=params.batch_size, readout=params.readout)
...
if __name__ == '__main__':
    hyperparameter_defaults = dict(
        root='/media/sdd/hfaure/riboset/RiboHybrid2',
        seed=65,
        num_workers=4,
        batch_size=64,
        epochs=70,
