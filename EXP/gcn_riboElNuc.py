...
class LitGCN2(pl.LightningModule):
    """
    ...
    def __init__(self, batch_size=32):
        ...
        self.lit_layers = gcnSeq('x, edge_index, batch', [
            (GCN2(in_channels=126), 'x, edge_index, batch -> x_gcn'),
            (MLP(filters=[320, 128, 64, 1], dropout=0.3), 'x_gcn -> x_out')
        ])
        self.best_r2 = -np.inf
...
def train_main_gcn(params):
    ...
    ribonet = LitGCN2(batch_size=params.batch_size)
...
if __name__ == '__main__':
    hyperparameter_defaults = dict(
        root='/media/sdd/hfaure/riboset/RiboElNuc',
        seed=65,
        num_workers=4,
        batch_size=64,
        epochs=50,
